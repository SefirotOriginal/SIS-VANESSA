                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="reference_number" id="reference_number" class="form-control"
                            value="{{ $purchase->reference_number }}" required>
                    </div>
                    <div class="col-md-4">
                        <label for="receipt_type" class="form-label">Tipo de Recibo</label>
                        <select name="receipt_type" id="receipt_type" class="form-control">
                            <option value="Factura" {{ $purchase->receipt_type == 'Factura' ? 'selected' : '' }}>Factura
                            </option>
                            <option value="Nota" {{ $purchase->receipt_type == 'Nota' ? 'selected' : '' }}>Nota de
                                Remisión</option>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#productSelect').on('change', function() {
                ...
                if (currentBatches.length > 0) {
                    currentBatches.forEach(batch => {
                        let option = $('<option>').attr('value', batch.batch_number).text(
                            `Expira: ${batch.expiration_date}`);
            ...
            $('#btnAddRow').click(function() {
                ...
                if (!batch) {
                    Swal.fire('Error', 'Escribe el lote', 'warning');
                    return;
                }
                if (!expiration) {
                    Swal.fire('Error', 'Fecha caducidad requerida', 'warning');
                    return;
                }
                if (isNaN(cost) || cost < 0) {
                    Swal.fire('Error', 'Costo inválido', 'warning');
                    return;
                }
                ...
                purchaseTable.rows().every(function() {
                    ...
                    if (currentId == id && currentBatch === batch && Math.abs(currentCost - cost) <
                        0.01) {
            ...
            function initializeSave() {
                btnSave.addEventListener('click', function() {
                    ...
                    if ($('#purchaseTable tbody tr').length === 0) {
                        Swal.fire('Error', 'Agrega productos', 'warning');
                        return;
                    }
                ...
                $('#purchaseTable tbody tr').each(function(index) {
                    ...
                    if (detailId) {
                        form.insertAdjacentHTML('beforeend',
                            `<input type="hidden" name="items[${index}][detail_id]" value="${detailId}">`
                        );
                    }

                    form.insertAdjacentHTML('beforeend',
                        `<input type="hidden" name="items[${index}][product_presentation_id]" value="${id}">`
                    );
                    form.insertAdjacentHTML('beforeend',
                        `<input type="hidden" name="items[${index}][batch_number]" value="${batch}">`);
                    form.insertAdjacentHTML('beforeend',
                        `<input type="hidden" name="items[${index}][expiration_date]" value="${expiration}">`
                    );
                    form.insertAdjacentHTML('beforeend',
                        `<input type="hidden" name="items[${index}][purchase_price]" value="${cost}">`);
                    form.insertAdjacentHTML('beforeend',
