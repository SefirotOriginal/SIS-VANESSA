    ...
    <script>
        ...
        $(document).ready(function() {
                ...
                if (currentBatches.length > 0) {
                    currentBatches.forEach(batch => {
                        let option = $('<option>').attr('value', batch.batch_number).text(
                            `Expira: ${batch.expiration_date} (Stock: ${batch.stock})`);
            ...
            $('#btnAddRow').click(function() {
                ...
                if (!id) {
                    Swal.fire('Falta información', 'Selecciona un producto', 'warning');
                    return;
                }
                if (!batch) {
                    Swal.fire('Falta información', 'Escribe el número de lote', 'warning');
                    return;
                }
                if (!expiration) {
                    Swal.fire('Falta información', 'Selecciona la fecha de caducidad', 'warning');
                    return;
                }
                if (isNaN(cost) || cost <= 0) {
                    Swal.fire('Error', 'Ingresa un costo válido', 'warning');
                    return;
                }
                ...
                purchaseTable.rows().every(function() {
                    ...
                    if (currentId == id && currentBatch === batch && currentExp === expiration &&
                        currentCost === cost) {
                ...
                if (!found) {
                    ...
                    let qtyHTML =
                        `<span class="qty-val font-weight-bold" style="font-size:1.1em">${qtyToAdd}</span>`;
